...
export class UserController {
  ...
  resetPin = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const data = await userService.resetPin(userId, { currentPin, newPin });
  ...
  setSecretPin = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?.id;
      if (!userId) throw new UnauthorizedError("User not authenticated");

      const { pin } = req.body;
      const data = await userService.setSecretPin(userId, pin);
  ...
  toggleSecretPin = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?.id;
      if (!userId) throw new UnauthorizedError("User not authenticated");

      const { enabled } = req.body;
      const data = await userService.toggleSecretPin(userId, enabled);
